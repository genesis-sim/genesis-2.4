...
// Uses the variable object in the diskio library 
// to store data in netcdf binary file 
...
create diskio /dout -filename testfile.nc -accessmode w -fileformat netcdf
setfield ^ append 1
for(i=0; i<100; i=i+1)
	create variable /dout/outputvalue[{i}] -datatype float32 
	addmsg /manytabs/tab[{i}] /dout/outputvalue[{i}] SAVE output
end
reset
